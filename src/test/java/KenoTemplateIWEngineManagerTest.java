...
public class KenoTemplateIWEngineManagerTest
{
	...
	private long[] spots1PricePoint100NoWinForce = {
		5,
	};
	private long[] spots1PricePoint100Division1Force = {
		0,
	};
	...
	{
		logger = Logger.getLogger(KenoTemplateIWEngineManagerTest.class);
		gameParams = XMLUtil.parseResource("paymodel/KenoTemplateIW/GameParams.xml").getDocumentElement();
		previousOutcome = XMLUtil.parseResource("paymodel/KenoTemplateIW/InitOutcome.xml").getDocumentElement();
			...
			throws Exception
	{
		...
		playerInput = new PlayerInputBuilder()
				...
				.appendJSONInput("{'testData':0, 'spots': 1}").build();
		...
		// Scenario
		TestRandomNumberProxy.setTestRngData(spots1PricePoint100NoWinForce);
		...
		// ScenarioLoop
		TestRandomNumberProxy.setTestRngData(0);
		TestGDSProxy.setTestGdsData(getTestGdsData("test-scenario-data"));
		playerInput = new PlayerInputBuilder().appendActionInput("play").build();
		outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
		outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
	...
	@Test
	public void test_buy_100x_25000CreditsWin_RevealThreeTimes()
		throws Exception
	{
		Element playerInput;
		int pricePoint = 100;
		...
		// Scenario
		TestRandomNumberProxy.setTestRngData(spots1PricePoint100Division1Force);
		TestGDSProxy.setTestGdsData(getTestGdsData("test-scenario-data"));
		playerInput = new PlayerInputBuilder().appendActionInput("play").build();
		outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
		outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
		...
		assertEquals("Wager", JXPathUtil.getString(outcome, "//NextStage/text()"));
		assertSettledPendingPayout(outcome, pricePoint, 0, 10000);
	}
			...
			throws Exception
	{
		...
		assertEquals("WagerLoop", JXPathUtil.getString(outcome, "//NextStage/text()"));
		assertSettledPendingPayout(outcome, 0, pricePoint, 0);
	...
	@Test
	public void test_try_100x_25000CreditsWin()
			throws Exception
	{
		Element playerInput;
		int pricePoint = 100;
