...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="PricePoint" mode="PricePoint">
		...
		<xsl:variable name="spots" select="@spots"/>
		<xsl:variable name='divisionStripName' select="concat('DivisionSelection', $spots, '_', $pricePoint)" />
		<xsl:text>{</xsl:text>
		...
		<xsl:apply-templates mode="PrizeStructure" select="//InstantWinPrizeInfo[@name='PrizeStructure']/PricePoint[@amount=$pricePoint and @spots=$spots]/Prize">
			<xsl:with-param name="divisionStripName" select="$divisionStripName"/>
		</xsl:apply-templates>
		<xsl:text>],</xsl:text>
		<xsl:text>"prizeTable":[</xsl:text>
		<xsl:apply-templates mode="PrizeTable" select="//InstantWinPrizeInfo[@name='PrizeTable']/PricePoint[@amount=$pricePoint and @spots=$spots]/Prize"/>
		<xsl:text>]</xsl:text>
		<xsl:text>}</xsl:text>
